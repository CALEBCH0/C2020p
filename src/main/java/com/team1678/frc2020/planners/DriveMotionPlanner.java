...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public DriveMotionPlanner() {
        final DCMotorTransmission transmission = new DCMotorTransmission(
            1.0 / Constants.kDriveKv,       // Speed per volt
            Math.pow(Units.inches_to_meters(Constants.kDriveWheelRadiusInches), 2) * Constants.kRobotLinearInertia / (2.0 * Constants.kDriveKa),    // Torque per volt
            Constants.kDriveVIntercept);    // Friction voltage 
        mModel = new DifferentialDrive(
                Constants.kRobotLinearInertia,      // Mass
                Constants.kRobotAngularInertia,     // Moi
                Constants.kRobotAngularDrag,        // Angular Drag 
                Units.inches_to_meters(Constants.kDriveWheelDiameterInches / 2.0),      // Wheel Radius
                Units.inches_to_meters(Constants.kDriveWheelTrackWidthInches / 2.0) * Constants.kTrackScrubFactor,      // Effective wheelbase radius
                transmission,       // Left transmission
                transmission        // Right transmission
        );
    }
    ...
    public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
        ...
        // Spline interpolation: given the points, calculate the smoother path often using polymonials 
    ...
    public String toCSV() {
        ...
        return fmt.format(
                mOutput.left_velocity) + fmt.format(mOutput.right_velocity) + "," 
    ...
    /** 
     * Implements Curvature-Velocity Method (CVM) for Differential Steered (DS) robots.
     */ 
    //   until goal is reached
    //   for all vL
    //   for all vR
    //   f = angular1 * speed(vL, vR) + angular2 * distance(vL, vR) + angular3 * head(vL, vR)
    //   if f > fMax then fMax = f(save vL and vR)
    protected Output updatePID(DifferentialDrive.DriveDynamics dynamics, Pose2d currrent_state) {
        ...
        adjusted_velocity.angular = dynamics.chassis_velocity.angular + dynamics.chassis_velocity.linear * kPathKY * 
                Units.inches_to_meters(mError.getTranslation().y()) + kPathTheta * mError.getRotation().getRadians(); 
        ...
        // Compute adjusdted left and right wheel velocities
    ...
    /**
     * A path tracking algorithm that uses lookahead points to guide the robot.
     * The robot constantly chases the point in front of it, called lookahead point. 
     * Once it reaches the point, it moves to the next lookahead point.
     * Given the pose of the robot, it calculates the linear and angular velocities.
     * @param dynamics
     * Differntial steering control of the robot wheels
     * @param current_state
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        if (actual_lookahead_distance < Constants.kPathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(
                    new Pose2dWithCurvature(
                            lookahead_state.state().getPose().transformBy(Pose2d.fromTranslation(new Translation2d(
                                    (mIsReversed ? -1.0 : 1.0) * (Constants.kPathMinLookaheadDistance - actual_lookahead_distance)
                                    , 0.0)))
                            , 0.0)
                    , lookahead_state.t()
        ...
        // Use pure pursuit to peek ahead along the trakectory and generate a new curvature
    ...
    protected Output updateNonlinearFeedback(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        DifferentialDrive.WheelState feedforward_voltage = mModel.solveInverseDynamics(dynamics.chassis_velocity, dynamics.chassis_acceleration).voltage;
        ...
        return new Output(
                ...
                dynamics.wheel_acceleration.left, 
                dynamics.wheel_acceleration.right, 
                feedforward_voltage.left, 
                feedforward_voltage.right);
    }

    public Output update(double timestamp, Pose2d current_state) {
        if (mCurrentTrajectory == null) return new Output();

        if (mCurrentTrajectory.getProgress() == 0.0 && !Double.isInfinite(mLastTime)) {
            mLastTime = timestamp;
        }
        ...
        if (mCurrentTrajectory.isDone()) {
            // Gernerate feedforward voltages
